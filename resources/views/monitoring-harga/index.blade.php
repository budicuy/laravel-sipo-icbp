...
@section('content')
    <div class="p-6 bg-gray-50 min-h-screen">
            ...
            <form method="GET" action="{{ route('monitoring.harga.index') }}">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            <select name="months"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent appearance-none bg-white pr-10">
                    ...
                    <div class="flex items-end">
                        <button type="submit"
                            class="w-full px-6 py-2.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-medium rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2">
                    ...
                    @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                        <div class="flex items-end">
                            <a href="{{ route('monitoring.harga.export', ['months' => $months]) }}"
                                class="w-full px-6 py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-medium rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <!-- Total Obat -->
            <div
                class="relative overflow-hidden bg-gradient-to-br from-blue-500 via-blue-600 to-blue-700 rounded-xl p-6 text-white">
            ...
            <!-- Harga Terkini -->
            <div
                class="relative overflow-hidden bg-gradient-to-br from-green-500 via-green-600 to-green-700 rounded-xl p-6 text-white">
                <div class="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-white opacity-10 rounded-full"></div>
                <div class="relative">
                    <div class="flex items-center gap-2 mb-2">
            ...
            <!-- Harga Kadaluarsa -->
            <div
                class="relative overflow-hidden bg-gradient-to-br from-amber-500 via-amber-600 to-amber-700 rounded-xl p-6 text-white">
                <div class="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-white opacity-10 rounded-full"></div>
                <div class="relative">
                    <div class="flex items-center gap-2 mb-2">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                {{--
                    <button onclick="generateRecommendations()"
                        class="px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-medium rounded-lg transition-all shadow-md hover:shadow-lg flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr class="bg-gradient-to-r from-gray-800 to-gray-900">
                                <th class="px-4 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">ID
                                    Obat</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($obatStaleHarga as $obat)
                                <tr class="hover:bg-amber-50 transition-colors">
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                            <button onclick="viewHargaHistory({{ $obat->id_obat }})"
                                                class="inline-flex items-center px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-all mr-2">
                                            ...
                                            <a href="{{ route('harga-obat.edit', $obat->id_obat) }}"
                                        ...
                                        @else
                                            <span class="text-sm text-gray-400 italic">Tidak ada aksi</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-gray-500">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <span class="text-xs text-gray-400 mt-1">Semua obat memiliki harga yang
                                                terkini</span>
                                        </div>
                                    </td>
                                </tr>
        ...
        <script>
            function generateRecommendations() {
                ...
                fetch(`/monitoring/harga/recommendations?target_periode=${targetPeriode}`)
                    ...
                    .then(data => {
                        let content =
                            '<div class="mb-4"><p class="text-sm text-gray-600">Rekomendasi harga untuk periode <strong>' +
                            targetPeriode + '</strong></p></div>';

                        if (data.length === 0) {
                            content +=
                                '<div class="text-center py-8 text-gray-500"><p>Tidak ada rekomendasi harga untuk periode ini.</p></div>';
                        } else {
                    ...
                    `;
                            ...
                            content +=
                                '<button type="button" onclick="closeRecommendationsModal()" class="px-4 py-2 bg-gray-300 hover:bg-gray-400 text-gray-800 text-sm font-medium rounded-lg transition-all">Batal</button>';
                            content +=
                                '<button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-all">Buat Harga</button>';
                            content += '</div>';
                            ...
                            for (let i = 0; i < data.length; i++) {
                                ...
                                if (checkbox) {
                                    recommendations.push({
                                        ...
                                        harga_per_satuan: parseFloat(formData.get(
                                            `recommendations[${i}][harga_per_satuan]`)),
                                        harga_per_kemasan: parseFloat(formData.get(
                                            `recommendations[${i}][harga_per_kemasan]`)),
                                        jumlah_per_kemasan: parseInt(formData.get(
                                            `recommendations[${i}][jumlah_per_kemasan]`))
                                    });
                                }
                            }
            ...
            function viewHargaHistory(idObat) {
                fetch(`/monitoring/harga/history/${idObat}`)
                    ...
                    .then(data => {
                        ...
                        content +=
                            '<th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Harga Kemasan</th>';
                        content +=
                        ...
                        if (data.length === 0) {
                            content +=
                                '<tr><td colspan="4" class="px-4 py-4 text-center text-gray-500">Tidak ada histori harga</td></tr>';
                        } else {
                            data.forEach(item => {
                                content += `
                        <tr>
