...
<!-- Filter Section -->
<div class="p-6 bg-gradient-to-r {{ $currentTheme['bgGradient'] }} border-b border-gray-200">
    ...
    <form method="{{ $method }}" action="{{ $action }}" class="grid grid-cols-1 {{ $gridCols }} gap-4">
        @foreach($fields as $field)
            ...
            @elseif($field['type'] === 'per_page')
                @if($showPerPage)
                <div class="{{ $field['colSpan'] ?? 'md:col-span-1' }}">
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ $field['label'] ?? 'Data per Halaman' }}</label>
        ...
        <div class="md:col-span-1 flex items-end gap-2">
            <button type="submit" class="flex-1 px-5 py-2.5 bg-gradient-to-r {{ $currentTheme['buttonGradient'] }} {{ $currentTheme['buttonHoverGradient'] }} text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                Filter
            </button>
            <a href="{{ $resetUrl }}" class="px-5 py-2.5 bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
