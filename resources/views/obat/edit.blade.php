...
<div class="p-6 bg-gray-50 min-h-screen" x-data="{
        ...
        <div class="flex items-center gap-3 mb-3">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
                    <div class="bg-linear-to-r from-yellow-500 to-yellow-600 p-3 rounded-lg shadow-lg">
    ...
    <form action="{{ route('obat.update', $obat->id_obat) }}" method="POST">
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
            <div class="bg-linear-to-r from-yellow-500 to-yellow-600 px-6 py-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="id_satuan" name="id_satuan" required x-model="id_satuan"
                            ...
                            <option value="{{ $satuan->id_satuan }}" {{ old('id_satuan', $obat->id_satuan) ==
                                $satuan->id_satuan ? 'selected' : '' }}>
                                {{ $satuan->nama_satuan }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required
                            ...
                            <option value="aktif" {{ old('status', $obat->status) == 'aktif' ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="non-aktif" {{ old('status', $obat->status) == 'non-aktif' ? 'selected' : ''
                                }}>Non-Aktif</option>
                        </select>
                        @error('status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                <a href="{{ route('obat.index') }}"
                    class="px-6 py-2.5 bg-linear-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                    Batal
                </a>
                <button type="submit"
                    class="px-6 py-2.5 bg-linear-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                    Update Data
                </button>
            </div>
        </div>
