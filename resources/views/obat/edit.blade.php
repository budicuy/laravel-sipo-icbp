...
<div class="p-6 bg-gray-50 min-h-screen"
     x-data="{
         ...
         jumlah_per_kemasan: {{ old('jumlah_per_kemasan', $obat->jumlah_per_kemasan) }},
         harga_per_kemasan: {{ old('harga_per_kemasan', $obat->harga_per_kemasan) }},
         harga_per_satuan: {{ old('harga_per_satuan', $obat->harga_per_satuan) }},
         stok_awal: {{ old('stok_awal', $obat->stok_awal) }},
         stok_masuk: {{ old('stok_masuk', $obat->stok_masuk) }},
         stok_keluar: {{ old('stok_keluar', $obat->stok_keluar) }},
         stok_akhir: {{ old('stok_akhir', $obat->stok_akhir) }},
    ...
    <form action="{{ route('obat.update', $obat->id_obat) }}" method="POST">
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="id_jenis_obat" name="id_jenis_obat" required
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 bg-white @error('id_jenis_obat') border-red-500 @enderror">
                            <option value="">Pilih Jenis Obat</option>
                            @foreach($jenisObats as $jenis)
                                <option value="{{ $jenis->id_jenis_obat }}" {{ old('id_jenis_obat', $obat->id_jenis_obat) == $jenis->id_jenis_obat ? 'selected' : '' }}>
                                    {{ $jenis->nama_jenis }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="id_satuan" name="id_satuan" required x-model="id_satuan"
                            ...
                            @foreach($satuanObats as $satuan)
                                <option value="{{ $satuan->id_satuan }}" {{ old('id_satuan', $obat->id_satuan) == $satuan->id_satuan ? 'selected' : '' }}>
                                    {{ $satuan->nama_satuan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="jumlah_per_kemasan" name="jumlah_per_kemasan" x-model="jumlah_per_kemasan" required
                                ...
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('jumlah_per_kemasan') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="number" id="stok_awal" name="stok_awal" value="{{ old('stok_awal', $obat->stok_awal) }}" required min="0" x-model="stok_awal"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('stok_awal') border-red-500 @enderror"
                               placeholder="0">
                        @error('stok_awal')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" id="stok_masuk" name="stok_masuk" value="{{ old('stok_masuk', $obat->stok_masuk) }}" required min="0" x-model="stok_masuk"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('stok_masuk') border-red-500 @enderror"
                               placeholder="0">
                        @error('stok_masuk')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" id="stok_keluar" name="stok_keluar" value="{{ old('stok_keluar', $obat->stok_keluar) }}" required min="0" x-model="stok_keluar"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('stok_keluar') border-red-500 @enderror"
                               placeholder="0">
                        @error('stok_keluar')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="harga_per_kemasan" name="harga_per_kemasan" value="{{ old('harga_per_kemasan', $obat->harga_per_kemasan) }}" required min="0" step="0.01" x-model="harga_per_kemasan"
                                   class="w-full pl-12 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('harga_per_kemasan') border-red-500 @enderror"
                                   placeholder="0">
                        </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                    Update Data
                </button>
            </div>
        </div>
