            ...
            <div class="p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-purple-50">
                <div class="flex flex-wrap gap-3 items-center">
                    ...
                    @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                        <a href="{{ route('keluarga.export', request()->query()) }}"
                            class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <form method="GET" class="flex flex-wrap items-end gap-3">
                    <div class="flex-1 min-w-[250px]">
                        ...
                        <input type="text" name="q" value="{{ request('q') }}"
                            ...
                            placeholder="NIK, Nama Karyawan, atau Nama Keluarga">
                    </div>

                    <div class="min-w-[150px]">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Jenis Kelamin</label>
                        <select name="jenis_kelamin"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm bg-white">
                    ...
                    <div class="min-w-[180px]">
                        ...
                        <select name="kode_hubungan"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm bg-white">
                            <option value="">Semua</option>
                            @foreach ($hubungans as $hubungan)
                                <option value="{{ $hubungan->kode_hubungan }}"
                                    {{ request('kode_hubungan') == $hubungan->kode_hubungan ? 'selected' : '' }}>
                                    {{ $hubungan->hubungan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex gap-2">
                        <button type="submit"
                            class="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                            ...
                            <tr class="hover:bg-purple-50 transition-colors">
                                @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <input name="ids[]" value="{{ $keluarga->id_keluarga }}" type="checkbox"
                                            class="row-checkbox rounded border-gray-300 text-purple-600 focus:ring-2 focus:ring-purple-500">
                                    </td>
                                @endif
                                <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ ($keluargas->currentPage() - 1) * $keluargas->perPage() + $loop->iteration }}</td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm">
                                        <div class="font-semibold text-purple-600">
                                            {{ optional($keluarga->karyawan)->nik_karyawan }}</div>
                                        <div class="text-xs text-gray-500">
                                            {{ optional($keluarga->karyawan)->nama_karyawan }}</div>
                                    </div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                ...
                                @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                    <td class="px-4 py-4 whitespace-nowrap text-sm">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('keluarga.edit', $keluarga->id_keluarga) }}"
                                            ...
                                            <form action="{{ route('keluarga.destroy', $keluarga->id_keluarga) }}"
                    ...
                    <div class="px-6 py-5 border-t border-gray-200 bg-white">
                        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <nav class="flex items-center gap-2" role="navigation">
                                ...
                                @else
                                    <a href="{{ $keluargas->appends(request()->except('page'))->url(1) }}"
                                ...
                                @else
                                    <a href="{{ $keluargas->appends(request()->except('page'))->previousPageUrl() }}"
                                        class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">Previous</a>
                                @endif
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    @if ($start > 1)
                                        <a href="{{ $keluargas->appends(request()->except('page'))->url(1) }}"
                                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">1</a>
                                        @if ($start > 2)
                                    ...
                                    @for ($i = $start; $i <= $end; $i++)
                                        @if ($i == $keluargas->currentPage())
                                            <span
                                                class="px-3 py-2 text-sm font-bold text-white bg-gradient-to-r from-purple-600 to-purple-700 rounded-lg shadow-md">{{ $i }}</span>
                                        @else
                                            <a href="{{ $keluargas->appends(request()->except('page'))->url($i) }}"
                                                class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">{{ $i }}</a>
                                        @endif
                                    @endfor
                                    ...
                                    @if ($end < $keluargas->lastPage())
                                        @if ($end < $keluargas->lastPage() - 1)
                                            <span class="px-2 text-gray-500">...</span>
                                        @endif
                                        <a href="{{ $keluargas->appends(request()->except('page'))->url($keluargas->lastPage()) }}"
                                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">{{ $keluargas->lastPage() }}</a>
                                    @endif
                                </div>

                                @if ($keluargas->hasMorePages())
                                    <a href="{{ $keluargas->appends(request()->except('page'))->nextPageUrl() }}"
                                        class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">Next</a>
                                @else
                                    <span
    ...
    @push('scripts')
        <script>
            ...
            <div class="text-left">
                ...
                <div class="mb-4">
                    <a href="{{ route('keluarga.downloadTemplate') }}"
                ...
                <form id="importForm" action="{{ route('keluarga.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-4">
