        ...
        <form action="{{ route('kunjungan.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flex items-end gap-3">
                    <button type="submit"
                        class="flex-1 bg-linear-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white font-medium px-6 py-2.5 rounded-lg shadow-md hover:shadow-lg transition-all">
                    ...
                    <a href="{{ route('kunjungan.index') }}"
                        class="flex-1 bg-white hover:bg-gray-50 border-2 border-gray-300 text-gray-700 font-medium px-6 py-2.5 rounded-lg transition-all text-center">
                        <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-orange-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <a href="{{ route('kunjungan.detail', $patientGroup['latest_visit']->id_kunjungan) }}"
        ...
        <div class="px-6 py-5 border-t border-gray-200 bg-white">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <nav class="flex items-center gap-2" role="navigation">
                    ...
                    @else
                    <a href="{{ $kunjunganCollection->appends(request()->except('page'))->url(1) }}"
                        class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-orange-50 hover:border-orange-400 transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
                        </svg>
                    ...
                    @else
                    <a href="{{ $kunjunganCollection->appends(request()->except('page'))->previousPageUrl() }}"
                        class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-orange-50 hover:border-orange-400 transition-all">Previous</a>
                    @endif
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @if($start > 1)
                        <a href="{{ $kunjunganCollection->appends(request()->except('page'))->url(1) }}"
                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-orange-50 hover:border-orange-400 transition-all">1</a>
                        @if($start > 2)
                        <span class="px-2 text-gray-500">...</span>
                        @endif
                        @endif

                        @for($i = $start; $i <= $end; $i++) @if($i==$kunjunganCollection->currentPage())
                            <span
                                class="px-3 py-2 text-sm font-bold text-white bg-linear-to-r from-orange-500 to-red-600 rounded-lg shadow-md">{{
                                $i }}</span>
                            @else
                            <a href="{{ $kunjunganCollection->appends(request()->except('page'))->url($i) }}"
                                class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-orange-50 hover:border-orange-400 transition-all">{{
                                $i }}</a>
                            @endif
                            @endfor
                            ...
                            @if($end < $kunjunganCollection->lastPage())
                                @if($end < $kunjunganCollection->lastPage() - 1)
                                    <span class="px-2 text-gray-500">...</span>
                                    @endif
                                    <a href="{{ $kunjunganCollection->appends(request()->except('page'))->url($kunjunganCollection->lastPage()) }}"
                                        class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-orange-50 hover:border-orange-400 transition-all">{{
                                        $kunjunganCollection->lastPage() }}</a>
                                    @endif
                    </div>

                    @if($kunjunganCollection->hasMorePages())
                    <a href="{{ $kunjunganCollection->appends(request()->except('page'))->nextPageUrl() }}"
                        class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-orange-50 hover:border-orange-400 transition-all">Next</a>
                    @else
                    <span
