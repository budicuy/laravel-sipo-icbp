            ...
            <div class="p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-indigo-50">
                <div class="flex flex-wrap gap-3 items-center">
                    @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                        <a href="{{ route('harga-obat.create') }}"
                        ...
                        <a href="{{ route('harga-obat.export', request()->query()) }}"
                        ...
                        <a href="{{ route('harga-obat.import') }}"
                        ...
                        <a href="{{ route('harga-obat.template') }}"
                            class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-yellow-600 to-yellow-700 hover:from-yellow-700 hover:to-yellow-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                ...
                <form method="GET" class="flex flex-wrap gap-4 items-end">
                    <div class="flex-1 min-w-[200px]">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="obat" value="{{ request('obat') }}"
                        ...
                        <select name="periode"
                            ...
                            @foreach ($availablePeriodes as $periode)
                                <option value="{{ $periode['value'] }}"
                                    {{ request('periode') == $periode['value'] ? 'selected' : '' }}>{{ $periode['label'] }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex gap-2">
                        <button type="submit"
                            class="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <form method="GET" id="perPageForm" class="inline">
                        ...
                        <select name="per_page" onchange="document.getElementById('perPageForm').submit()"
                            class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm bg-white shadow-sm">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr class="bg-gradient-to-r from-indigo-700 to-purple-700">
                            @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                <th class="px-4 py-4 text-left">
                                    <input type="checkbox" onclick="toggleAll(this)"
                                        class="rounded border-gray-400 text-indigo-600 focus:ring-2 focus:ring-indigo-500">
                                </th>
                            @endif
                            ...
                            <th class="px-4 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Tanggal
                                Update</th>
                            @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                <th class="px-4 py-4 text-center text-xs font-bold text-white uppercase tracking-wider">
                                    Aksi
                        ...
                        @forelse($hargaObats as $index => $hargaObat)
                            <tr class="hover:bg-indigo-50 transition-colors">
                                @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                    <td class="px-4 py-3">
                                        <input type="checkbox" name="selected_ids[]"
                                            value="{{ $hargaObat->id_harga_obat }}"
                                            class="row-checkbox rounded border-gray-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500">
                                    </td>
                                @endif
                                <td class="px-4 py-3 text-sm text-gray-900">{{ $hargaObats->firstItem() + $index }}</td>
                                <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $hargaObat->obat->nama_obat }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600">
                                    {{ $hargaObat->obat->satuanObat->nama_satuan ?? '-' }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">
                                    <span
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600 text-center">
                                    {{ $hargaObat->jumlah_per_kemasan }}</td>
                                <td class="px-4 py-3 text-sm text-gray-900 text-center">Rp
                                    {{ number_format($hargaObat->harga_per_kemasan, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-sm text-gray-900 text-center">Rp
                                    {{ number_format($hargaObat->harga_per_satuan, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">
                                ...
                                @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                    <td class="px-4 py-3 text-sm font-medium text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('harga-obat.edit', $hargaObat->id_harga_obat) }}"
                                            ...
                                            <button
                                                onclick="deleteHargaObat({{ $hargaObat->id_harga_obat }}, '{{ $hargaObat->obat->nama_obat }} - {{ $hargaObat->periode }}')"
                    ...
                    <div class="px-6 py-5 border-t border-gray-200 bg-white">
                        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <nav class="flex items-center gap-2" role="navigation">
                                ...
                                @else
                                    <a href="{{ $hargaObats->appends(request()->except('page'))->url(1) }}"
                                        class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-indigo-50 hover:border-indigo-400 transition-all">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
                                        </svg>
                                ...
                                @else
                                    <a href="{{ $hargaObats->appends(request()->except('page'))->previousPageUrl() }}"
                                        class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-indigo-50 hover:border-indigo-400 transition-all">Previous</a>
                                @endif
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    @if ($start > 1)
                                        <a href="{{ $hargaObats->appends(request()->except('page'))->url(1) }}"
                                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-indigo-50 hover:border-indigo-400 transition-all">1</a>
                                        @if ($start > 2)
                                    ...
                                    @for ($i = $start; $i <= $end; $i++)
                                        @if ($i == $hargaObats->currentPage())
                                            <span
                                                class="px-3 py-2 text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-indigo-700 rounded-lg shadow-md">{{ $i }}</span>
                                        @else
                                            <a href="{{ $hargaObats->appends(request()->except('page'))->url($i) }}"
                                                class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-indigo-50 hover:border-indigo-400 transition-all">{{ $i }}</a>
                                        @endif
                                    @endfor
                                    ...
                                    @if ($end < $hargaObats->lastPage())
                                        @if ($end < $hargaObats->lastPage() - 1)
                                            <span class="px-2 text-gray-500">...</span>
                                        @endif
                                        <a href="{{ $hargaObats->appends(request()->except('page'))->url($hargaObats->lastPage()) }}"
                                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-indigo-50 hover:border-indigo-400 transition-all">{{ $hargaObats->lastPage() }}</a>
                                    @endif
                                </div>

                                @if ($hargaObats->hasMorePages())
                                    <a href="{{ $hargaObats->appends(request()->except('page'))->nextPageUrl() }}"
                                        class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-indigo-50 hover:border-indigo-400 transition-all">Next</a>
                                @else
                                    <span
