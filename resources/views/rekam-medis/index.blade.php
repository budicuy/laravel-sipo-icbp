        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 mb-6">
            <div class="border-b border-gray-200">
                <nav class="flex -mb-px">
                    <button type="button" onclick="switchTab('regular')" id="regular-tab"
                        class="tab-button py-4 px-6 border-b-2 font-medium text-sm rounded-tl-lg focus:outline-none transition-colors border-green-500 text-green-600 bg-green-50">
                        <div class="flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    ...
                    <button type="button" onclick="switchTab('emergency')" id="emergency-tab"
                        class="tab-button py-4 px-6 border-b-2 font-medium text-sm rounded-tr-lg focus:outline-none transition-colors border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">
                        <div class="flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <tr class="hover:bg-green-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                                        <div class="status-dropdown" data-id="{{ $rm->id_rekam }}">
                                            <select
                                                class="status-select px-3 py-1 rounded-full text-xs font-medium border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500
                                        @if ($rm->status == 'On Progress') bg-yellow-100 text-yellow-800
                                        @elseif($rm->status == 'Close') bg-green-100 text-green-800 @endif"
                                                data-id="{{ $rm->id_rekam }}"
                                                data-current-status="{{ $rm->status ?? 'On Progress' }}">
                                                <option value="On Progress"
                                                    {{ ($rm->status ?? 'On Progress') == 'On Progress' ? 'selected' : '' }}>
                                                    On Progress</option>
                                                <option value="Close"
                                                    {{ ($rm->status ?? 'On Progress') == 'Close' ? 'selected' : '' }}>Close
                                                </option>
                                            </select>
                                        </div>
                                    </td>
                                    ...
                                    @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex items-center justify-center space-x-2">
                                                <a href="{{ route('rekam-medis.edit', $rm->id_rekam) }}"
                    ...
                    <div class="px-6 py-5 border-t border-gray-200 bg-white">
                        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <nav class="flex items-center gap-2" role="navigation">
                                ...
                                @else
                                    <a href="{{ $rekamMedis->appends(request()->except('page'))->url(1) }}"
                                ...
                                @else
                                    <a href="{{ $rekamMedis->appends(request()->except('page'))->previousPageUrl() }}"
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    @for ($i = $start; $i <= $end; $i++)
                                        @if ($i == $rekamMedis->currentPage())
                                            <span
                                                class="px-3 py-2 text-sm font-bold text-white bg-gradient-to-r from-green-600 to-emerald-600 rounded-lg shadow-md">{{ $i }}</span>
                                        @else
                                    ...
                                    @if ($end < $rekamMedis->lastPage())
                                        @if ($end < $rekamMedis->lastPage() - 1)
                                            <span class="px-2 text-gray-500">...</span>
                                        @endif
                                        <a href="{{ $rekamMedis->appends(request()->except('page'))->url($rekamMedis->lastPage()) }}"
                                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-green-50 hover:border-green-400 transition-all">{{ $rekamMedis->lastPage() }}</a>
                                    @endif
                                </div>

                                @if ($rekamMedis->hasMorePages())
                                    <a href="{{ $rekamMedis->appends(request()->except('page'))->nextPageUrl() }}"
                                ...
                                <tr class="hover:bg-red-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                                        <div class="status-dropdown" data-id="{{ $rm->id_emergency }}">
                                            <select
                                                class="status-select-emergency px-3 py-1 rounded-full text-xs font-medium border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500
                                        @if ($rm->status == 'On Progress') bg-yellow-100 text-yellow-800
                                        @elseif($rm->status == 'Close') bg-green-100 text-green-800 @endif"
                                                data-id="{{ $rm->id_emergency }}"
                                                data-current-status="{{ $rm->status }}">
                                                <option value="On Progress"
                                                    {{ $rm->status == 'On Progress' ? 'selected' : '' }}>On Progress
                                                </option>
                                                <option value="Close" {{ $rm->status == 'Close' ? 'selected' : '' }}>
                                                    Close</option>
                                            </select>
                                        </div>
                                    </td>
                                    ...
                                    @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex items-center justify-center space-x-2">
                                                ...
                                                <button type="button"
                                                    class="bg-red-500 hover:bg-red-600 text-white p-1.5 rounded delete-btn-emergency"
                                                    data-id="{{ $rm->id_emergency }}"
                ...
                @if ($rekamMedisEmergency->hasPages())
                    <div class="px-6 py-5 border-t border-gray-200 bg-white">
                        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div class="text-sm text-gray-600">
                                Halaman <span
                                    class="font-semibold text-gray-900">{{ $rekamMedisEmergency->currentPage() }}</span>
                                dari <span
                                    class="font-semibold text-gray-900">{{ $rekamMedisEmergency->lastPage() }}</span>
                                <span class="mx-2 text-gray-400">•</span>
                                Total <span class="font-semibold text-gray-900">{{ $rekamMedisEmergency->total() }}</span>
                                data
                            </div>
                            ...
                            <nav class="flex items-center gap-2" role="navigation">
                                ...
                                <div class="flex items-center gap-1">
                                    @php
                                        ...
                                        $end = min(
                                            $rekamMedisEmergency->currentPage() + 2,
                                            $rekamMedisEmergency->lastPage(),
                                        );
                                    @endphp
                                    ...
                                    @for ($i = $start; $i <= $end; $i++)
                                        @if ($i == $rekamMedisEmergency->currentPage())
                                            <span
                                                class="px-3 py-2 text-sm font-bold text-white bg-gradient-to-r from-red-600 to-pink-600 rounded-lg shadow-md">{{ $i }}</span>
                                        @else
                                    ...
                                    @if ($end < $rekamMedisEmergency->lastPage())
                                        ...
                                        <a href="{{ $rekamMedisEmergency->appends(request()->except('page'))->url($rekamMedisEmergency->lastPage()) }}"
                                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">{{ $rekamMedisEmergency->lastPage() }}</a>
                                    @endif
                                </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                        ...
                        Swal.fire({
                            ...
                            preConfirm: function() {
                                return new Promise(function(resolve) {
                                    ...
                                    form.method = 'POST';
                                    form.action =
                                        `{{ route('rekam-medis.destroy', ':id') }}`
                                        .replace(':id', id);

                                    // Add CSRF token
                                    const csrfToken = document.createElement(
                                        'input');
                        ...
                        Swal.fire({
                            ...
                            preConfirm: function() {
                                return new Promise(function(resolve) {
                                    ...
                                    form.method = 'POST';
                                    form.action =
                                        `{{ route('rekam-medis-emergency.destroy', ':id') }}`
                                        .replace(':id', id);
                        ...
                        fetch(`{{ route('rekam-medis.updateStatus', ':id') }}`.replace(':id', id), {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    showNotification(data.message || 'Gagal memperbarui status',
                                        'error');
                                }
                            })
                            .catch(error => {
                ...
                document.querySelectorAll('.status-select-emergency').forEach(select => {
                    select.addEventListener('change', function() {
                        ...
                        // Send AJAX request
                        fetch(`{{ route('rekam-medis-emergency.updateStatus', ':id') }}`.replace(':id',
                                id), {
                                method: 'PATCH',
                                headers: {
                                    'Content-Type': 'application/json',
        ...
        `;
            ...
            function switchTab(tabName) {
                ...
                document.querySelectorAll('.tab-button').forEach(button => {
                    button.classList.remove('border-green-500', 'text-green-600', 'bg-green-50', 'border-red-500',
