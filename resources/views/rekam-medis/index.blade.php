                ...
                <table class="w-full">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-green-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                                {{ is_string($rm->tanggal_periksa) ? $rm->tanggal_periksa :
                                $rm->tanggal_periksa->format('d-m-Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                                {{ $rm->waktu_periksa ? (is_string($rm->waktu_periksa) ? $rm->waktu_periksa :
                                $rm->waktu_periksa->format('H:i')) : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                                <div class="status-dropdown" data-id="{{ $rm->id_rekam }}">
                                    <select class="status-select px-3 py-1 rounded-full text-xs font-medium border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500
                                        ...
                                        <option value="On Progress" {{ ($rm->status ?? 'On Progress') == 'On Progress' ?
                                            'selected' : '' }}>
                                            On Progress</option>
                                        <option value="Close" {{ ($rm->status ?? 'On Progress') == 'Close' ? 'selected'
                                            : '' }}>Close
                                        </option>
                                    </select>
                                </div>
    ...
    <div id="emergency-content" class="tab-content hidden">
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                                <div class="status-dropdown" data-id="{{ $rm->id_emergency }}">
                                    <select class="status-select-emergency px-3 py-1 rounded-full text-xs font-medium border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500
                                        @if ($rm->status == 'On Progress') bg-yellow-100 text-yellow-800
                                        @elseif($rm->status == 'Close') bg-green-100 text-green-800 @endif"
                                        data-id="{{ $rm->id_emergency }}" data-current-status="{{ $rm->status }}">
                                        <option value="On Progress" {{ $rm->status == 'On Progress' ? 'selected' : ''
                                            }}>On Progress
                                        </option>
            ...
            <div class="px-6 py-5 border-t border-gray-200 bg-white">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div class="text-sm text-gray-600">
                        Halaman <span class="font-semibold text-gray-900">{{ $rekamMedisEmergency->currentPage()
                            }}</span>
