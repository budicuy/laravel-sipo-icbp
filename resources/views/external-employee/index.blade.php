    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-purple-50">
            <div class="flex flex-wrap gap-3 items-center">
                <a href="{{ route('external-employee.create') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
        ...
        <x-filter-section
            theme="purple-indigo"
            action="{{ route('external-employee.index') }}"
            reset-url="{{ route('external-employee.index') }}"
            :fields="[
                [
                    'type' => 'text',
                    'name' => 'search',
                ...
                [
                    ...
                    'options' => array_merge(
                        ...
                        $vendors->map(fn($vendor) => ['value' => $vendor->id_vendor, 'label' => $vendor->nama_vendor])->toArray()
                    ),
                    'colSpan' => 'md:col-span-1'
                ],
                [
                    ...
                    'options' => array_merge(
                        ...
                        $kategoris->map(fn($kategori) => ['value' => $kategori->id_kategori, 'label' => $kategori->nama_kategori])->toArray()
                    ),
                    'colSpan' => 'md:col-span-1'
                ]
            ]"
        ...
        <div class="p-5 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
            ...
            <div class="text-sm text-gray-600">
                Total: <span class="font-semibold text-gray-900">{{ $externalEmployees->total() }}</span> external employee
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-purple-50 transition-colors">
                        <td class="px-4 py-4 whitespace-nowrap">
                            <input name="ids[]" value="{{ $employee->id }}" type="checkbox" class="row-checkbox rounded border-gray-300 text-purple-600 focus:ring-2 focus:ring-purple-500" data-id="{{ $employee->id }}">
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ ($externalEmployees->currentPage() - 1) * $externalEmployees->perPage() + $loop->iteration }}</td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <span class="text-sm font-semibold text-purple-600">{{ $employee->nik_employee }}</span>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                @if($employee->foto)
                                    ...
                                    <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full items-center justify-center text-white text-xs font-bold hidden">
                                        {{ strtoupper(substr($employee->nama_employee, 0, 1)) }}
                                    </div>
                                @else
                                    <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm">
                                        {{ strtoupper(substr($employee->nama_employee, 0, 1)) }}
                                    </div>
                                @endif
                                <span class="text-sm font-medium text-gray-900">{{ $employee->nama_employee }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{{ $employee->vendor->nama_vendor }}</td>
                        <td class="px-4 py-4 whitespace-nowrap">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('external-employee.show', $employee->id) }}" class="inline-flex items-center justify-center w-9 h-9 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-all shadow-sm hover:shadow-md" title="Detail">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                    </svg>
                                </a>
                                <a href="{{ route('external-employee.edit', $employee->id) }}" class="inline-flex items-center justify-center w-9 h-9 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-all shadow-sm hover:shadow-md" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form action="{{ route('external-employee.destroy', $employee->id) }}" method="POST" class="delete-form">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="px-6 py-5 border-t border-gray-200 bg-white">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                <!-- Info Text -->
                <div class="text-sm text-gray-600">
                    Halaman <span class="font-semibold text-gray-900">{{ $externalEmployees->currentPage() }}</span>
                ...
                <nav class="flex items-center gap-2" role="navigation" aria-label="Pagination Navigation">
                    ...
                    @else
                        <a href="{{ $externalEmployees->appends(request()->except('page'))->url(1) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/>
                            </svg>
                        </a>
                    ...
                    @else
                        <a href="{{ $externalEmployees->appends(request()->except('page'))->previousPageUrl() }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">
                            Previous
                        </a>
                    @endif
                    ...
                    <div class="flex items-center gap-1">
                        @php
                            ...
                            $end = min($externalEmployees->currentPage() + 2, $externalEmployees->lastPage());
                        @endphp
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            @if($i == $externalEmployees->currentPage())
                                <span class="px-3 py-2 text-sm font-bold text-white bg-gradient-to-r from-purple-600 to-purple-700 rounded-lg shadow-md">
                                    {{ $i }}
                                </span>
                            @else
                                <a href="{{ $externalEmployees->appends(request()->except('page'))->url($i) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">
                                    {{ $i }}
                                </a>
                            @endif
                        @endfor
                        ...
                        @if($end < $externalEmployees->lastPage())
                            @if($end < $externalEmployees->lastPage() - 1)
                                <span class="px-2 text-gray-500">...</span>
                            @endif
                            <a href="{{ $externalEmployees->appends(request()->except('page'))->url($externalEmployees->lastPage()) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">
                    ...
                    @if($externalEmployees->hasMorePages())
                        <a href="{{ $externalEmployees->appends(request()->except('page'))->nextPageUrl() }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:border-purple-400 transition-all">
                            Next
                        </a>
                    @else
                        <span class="px-4 py-2 text-sm font-medium text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed">
