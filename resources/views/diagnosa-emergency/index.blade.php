    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-red-50">
            <div class="flex flex-wrap gap-3 items-center">
                <a href="{{ route('diagnosa-emergency.create') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                ...
                <a href="{{ route('diagnosa.index') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6 bg-gradient-to-r from-red-50 to-pink-50 border-b border-gray-200">
            ...
            <form method="GET" action="{{ route('diagnosa-emergency.index') }}" class="flex flex-wrap gap-4 items-end">
                <div class="flex-1 min-w-[300px]">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" class="pl-10 pr-4 py-2 w-full border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 text-sm" placeholder="Masukkan nama diagnosa emergency...">
                    </div>
                </div>
                ...
                <a href="{{ route('diagnosa-emergency.index') }}" class="px-5 py-2.5 bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                    <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-5 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
            ...
            <div class="text-sm text-gray-600">
                Total: <span class="font-semibold text-gray-900">{{ $diagnosaEmergencies->total() }}</span> diagnosa emergency
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-red-50 transition-colors">
                        <td class="px-4 py-4 whitespace-nowrap">
                            <input name="ids[]" value="{{ $diagnosa->id_diagnosa_emergency }}" type="checkbox" class="row-checkbox rounded border-gray-300 text-red-600 focus:ring-2 focus:ring-red-500">
                        </td>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-medium text-gray-900">{{ $diagnosa->nama_diagnosa_emergency }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('diagnosa-emergency.edit', $diagnosa->id_diagnosa_emergency) }}" class="inline-flex items-center justify-center w-9 h-9 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-all shadow-sm hover:shadow-md" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <button onclick="deleteDiagnosa({{ $diagnosa->id_diagnosa_emergency }}, '{{ $diagnosa->nama_diagnosa_emergency }}')" class="inline-flex items-center justify-center w-9 h-9 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-all shadow-sm hover:shadow-md" title="Hapus">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
        ...
        <div class="px-6 py-5 border-t border-gray-200 bg-white">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="text-sm text-gray-600">
                    Halaman <span class="font-semibold text-gray-900">{{ $diagnosaEmergencies->currentPage() }}</span>
                    dari <span class="font-semibold text-gray-900">{{ $diagnosaEmergencies->lastPage() }}</span>
                    <span class="mx-2 text-gray-400">•</span>
                    Total <span class="font-semibold text-gray-900">{{ $diagnosaEmergencies->total() }}</span> data
                </div>
                ...
                <nav class="flex items-center gap-2" role="navigation">
                    ...
                    @else
                        <a href="{{ $diagnosaEmergencies->appends(request()->except('page'))->url(1) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/>
                            </svg>
                        </a>
                    ...
                    @else
                        <a href="{{ $diagnosaEmergencies->appends(request()->except('page'))->previousPageUrl() }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">Previous</a>
                    @endif

                    <div class="flex items-center gap-1">
                        @php
                            $start = max($diagnosaEmergencies->currentPage() - 2, 1);
                            $end = min($diagnosaEmergencies->currentPage() + 2, $diagnosaEmergencies->lastPage());
                        @endphp

                        @if($start > 1)
                            <a href="{{ $diagnosaEmergencies->appends(request()->except('page'))->url(1) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">1</a>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            @if($i == $diagnosaEmergencies->currentPage())
                                <span class="px-3 py-2 text-sm font-bold text-white bg-gradient-to-r from-red-600 to-pink-700 rounded-lg shadow-md">{{ $i }}</span>
                            @else
                                <a href="{{ $diagnosaEmergencies->appends(request()->except('page'))->url($i) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">{{ $i }}</a>
                            @endif
                        @endfor
                        ...
                        @if($end < $diagnosaEmergencies->lastPage())
                            @if($end < $diagnosaEmergencies->lastPage() - 1)
                                <span class="px-2 text-gray-500">...</span>
                            @endif
                            <a href="{{ $diagnosaEmergencies->appends(request()->except('page'))->url($diagnosaEmergencies->lastPage()) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">{{ $diagnosaEmergencies->lastPage() }}</a>
                        @endif
                    </div>

                    @if($diagnosaEmergencies->hasMorePages())
                        <a href="{{ $diagnosaEmergencies->appends(request()->except('page'))->nextPageUrl() }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">Next</a>
                    @else
                    ...
                    @else
                        <a href="{{ $diagnosaEmergencies->appends(request()->except('page'))->url($diagnosaEmergencies->lastPage()) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"/>
                            </svg>
                        </a>
