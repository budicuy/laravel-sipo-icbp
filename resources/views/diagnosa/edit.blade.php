                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <select id="status" name="status"
                                    ...
                                    required>
                                    <option value="aktif"
                                        {{ old('status', $diagnosa->status ?? 'aktif') == 'aktif' ? 'selected' : '' }}>Aktif
                                    </option>
                                    <option value="non-aktif"
                                        {{ old('status', $diagnosa->status) == 'non-aktif' ? 'selected' : '' }}>Non-Aktif
                                    </option>
                                </select>
                            </div>
                                ...
                                @foreach ($obats as $obat)
                                    <div class="flex items-center mb-2">
                                        <input type="checkbox" id="obat_{{ $obat->id_obat }}" name="obat_ids[]"
                                            ...
                                            {{ in_array($obat->id_obat, old('obat_ids', $diagnosa->obats->pluck('id_obat')->toArray())) ? 'checked' : '' }}>
                                        <label for="obat_{{ $obat->id_obat }}" class="text-sm text-gray-700">
                                            {{ $obat->nama_obat }}
                                            @if ($obat->keterangan)
                                                <span
                                                    class="text-gray-500 text-xs">({{ Str::limit($obat->keterangan, 300) }})</span>
                                            @endif
                                        </label>
                                    </div>
                                @endforeach
