...
<div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-200 bg-linear-to-r from-gray-50 to-red-50">
            <div class="flex flex-wrap gap-3 items-center">
                ...
                <a href="{{ route('diagnosa-emergency.index') }}"
                    class="inline-flex items-center px-5 py-2.5 bg-linear-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('diagnosa.export', request()->query()) }}"
        ...
        <!-- Filter Section -->
        <div class="p-6 bg-linear-to-r from-red-50 to-pink-50 border-b border-gray-200">
            ...
            <form method="GET" action="{{ route('diagnosa.index') }}" class="flex flex-wrap gap-4 items-end">
                ...
                <button type="submit"
                    class="px-5 py-2.5 bg-linear-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead>
                    <tr class="bg-linear-to-r from-red-700 to-pink-700">
                        @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Super Admin')
                        ...
                        <th class="px-4 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">
                            <a href="{{ route('diagnosa.index', array_merge(request()->except(['page', 'sort', 'direction']), ['sort' => 'status', 'direction' => request('sort') == 'status' && request('direction') == 'asc' ? 'desc' : 'asc'])) }}"
                                class="flex items-center justify-between group hover:text-red-300 transition-colors">
                                <span>Status</span>
                                <span class="ml-2">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-red-50 transition-colors">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <input name="ids[]" value="{{ $diagnosa->id_diagnosa }}" type="checkbox"
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                <div
                                    class="w-8 h-8 bg-linear-to-br from-red-500 to-pink-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('diagnosa.edit', $diagnosa->id_diagnosa) }}"
                                ...
                                <button
                                    onclick="deleteDiagnosa({{ $diagnosa->id_diagnosa }}, '{{ $diagnosa->nama_diagnosa }}')"
        ...
        <div class="px-6 py-5 border-t border-gray-200 bg-white">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <nav class="flex items-center gap-2" role="navigation">
                    ...
                    @else
                    <a href="{{ $diagnosas->appends(request()->except('page'))->url(1) }}"
                    ...
                    @else
                    <a href="{{ $diagnosas->appends(request()->except('page'))->previousPageUrl() }}"
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @for ($i = $start; $i <= $end; $i++) @if ($i==$diagnosas->currentPage())
                            <span
                                class="px-3 py-2 text-sm font-bold text-white bg-linear-to-r from-red-600 to-red-700 rounded-lg shadow-md">{{
                                $i }}</span>
                            @else
                            <a href="{{ $diagnosas->appends(request()->except('page'))->url($i) }}"
                                class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">{{
                                $i }}</a>
                            @endif
                            @endfor
                            ...
                            @if ($end < $diagnosas->lastPage())
                                @if ($end < $diagnosas->lastPage() - 1)
                                    <span class="px-2 text-gray-500">...</span>
                                    @endif
                                    <a href="{{ $diagnosas->appends(request()->except('page'))->url($diagnosas->lastPage()) }}"
                                        class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-400 transition-all">{{
                                        $diagnosas->lastPage() }}</a>
                                    @endif
                    </div>

                    @if ($diagnosas->hasMorePages())
                    <a href="{{ $diagnosas->appends(request()->except('page'))->nextPageUrl() }}"
