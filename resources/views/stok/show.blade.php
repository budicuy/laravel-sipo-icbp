...
<div class="p-6 bg-gray-50 min-h-screen">
    ...
    <nav class="flex mb-6" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            <li class="inline-flex items-center">
                <a href="{{ route('stok.index') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600">
                ...
                <div>
                    <table class="w-full">
                        ...
                        <tr>
                            ...
                            <td class="py-2 text-sm text-gray-900">{{ number_format($obat->stok_awal) }} {{ $obat->satuanObat->nama_satuan ?? '' }}</td>
                        </tr>
                    </table>
                </div>
                <div>
                    <table class="w-full">
                        <tr>
                            ...
                            <td class="py-2">
                                <span class="text-xl font-bold {{ $sisaStok <= 0 ? 'text-red-600' : ($sisaStok <= 10 ? 'text-yellow-600' : 'text-green-600') }}">
            ...
            <form action="{{ route('stok.masuk.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" id="jumlah_stok_masuk" name="jumlah_stok_masuk" min="1" required
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="px-4 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($stok->stok_masuk > 0)
                                    <span class="text-green-600 font-semibold">+{{ number_format($stok->stok_masuk) }}</span>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($stok->stok_pakai > 0)
                                    <span class="text-red-600 font-semibold">-{{ number_format($stok->stok_pakai) }}</span>
                                @else
                                    <span class="text-gray-400">-</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <span class="{{ $stok->stok_akhir <= 0 ? 'text-red-600' : ($stok->stok_akhir <= 10 ? 'text-yellow-600' : 'text-green-600') }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <button onclick="openEditStokModal({{ $stok->id }}, '{{ $stok->periode }}', {{ $stok->tahun }}, {{ $stok->bulan }}, {{ $stok->stok_masuk }}, {{ $stok->stok_pakai }})"
        ...
        <div class="mt-6 bg-white rounded-xl shadow-md border border-gray-100 p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="text-center">
                    ...
                    <p class="text-2xl font-semibold text-green-600">{{ number_format($riwayatStok->sum('stok_masuk')) }}</p>
                </div>
                <div class="text-center">
                    <p class="text-sm font-medium text-gray-500">Total Stok Pakai</p>
                    <p class="text-2xl font-semibold text-red-600">{{ number_format($riwayatStok->sum('stok_pakai')) }}</p>
                </div>
                <div class="text-center">
                ...
                <div class="text-center">
                    ...
                    <p class="text-2xl font-semibold {{ $sisaStok <= 0 ? 'text-red-600' : ($sisaStok <= 10 ? 'text-yellow-600' : 'text-green-600') }}">
    ...
    <div class="mt-6 flex flex-wrap gap-3">
        <a href="{{ route('stok.index') }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium rounded-lg transition-colors flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg>
            Kembali
        </a>
        <a href="{{ route('obat.edit', $obat->id_obat) }}" class="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
            Edit Obat
...
<!-- Edit Stok Modal -->
<div id="editStokModal" class="fixed inset-0 bg-black/25 backdrop-blur-sm bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden transition-all duration-300">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div class="mt-2">
                            <form id="editStokForm" action="{{ route('stok.update') }}" method="POST">
                                ...
                                <input type="hidden" id="edit_obat_id" name="obat_id" value="{{ $obat->id_obat }}">
                                <input type="hidden" id="edit_tahun" name="tahun">
                                <input type="hidden" id="edit_bulan" name="bulan">
                                <input type="hidden" id="edit_stok_pakai" name="stok_pakai" value="{{ $stok->stok_pakai ?? 0 }}">

                                <div class="mb-4">
                                    <label for="edit_periode" class="block text-sm font-medium text-gray-700 mb-2">Periode</label>
                                ...
                                <div class="mb-4">
                                    <label for="edit_stok_masuk" class="block text-sm font-medium text-gray-700 mb-2">Stok Masuk</label>
                                    <div class="relative">
                                        ...
                                        <span class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-gray-500 text-sm">
