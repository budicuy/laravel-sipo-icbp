...
<div class="p-6 bg-gray-50 min-h-screen">
            ...
            <form action="{{ route('stok.masuk.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="flex items-end">
                        <button type="submit"
                            class="w-full px-4 py-2 bg-linear-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all">
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden mb-6">
        <div class="bg-linear-to-r from-indigo-600 to-purple-600 px-6 py-4 flex justify-between items-center">
            <h2 class="text-lg font-semibold text-white flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    ->where('bulan', '<=', $stok->bulan);
                                        ...
                                        <tr
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">{{ $stok->periode }}
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                @if ($stokAkhirBulanan <= 0) <span
                                                    class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                                    Habis
                                                    </span>
                                                    @elseif($stokAkhirBulanan <= 10) <span
                                                        class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                                                        Rendah
                                                        </span>
                                                        @else
...
<div id="editStokModal" class="fixed inset-0 bg-black/25 backdrop-blur-sm overflow-y-auto h-full w-full z-50 hidden">
    <div class="relative top-60 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <div class="px-7 py-3">
                <form id="editStokForm" method="POST" action="{{ route('stok.bulanan.update', ['id' => '__ID__']) }}">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="obat_id" value="{{ $obat->id_obat }}">
