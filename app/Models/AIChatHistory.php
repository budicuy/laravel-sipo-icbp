    ...
    class AIChatHistory extends Model
    {
        ...
        /**
         * Cache for total count (per-request)
         */
        protected static $totalCountCache = null;
        ...
        {
            if (self::$totalCountCache === null) {
                self::$totalCountCache = self::count();
            }
            return self::$totalCountCache;
        }

        /**
         * Cache for statistics (per-request)
         */
        protected static $statisticsCache = null;

        /**
         * Get statistics for admin dashboard (cached per request)
         */
        public static function getStatistics()
        {
            if (self::$statisticsCache !== null) {
                return self::$statisticsCache;
            }

            $totalUsers = self::getCachedCount();
        ...
        /**
         * Get user history with pagination (uses cached count to avoid duplicate count query)
         */
        public static function getUserHistory($perPage = 10)
        {
            $total = self::getCachedCount();
            $page = request()->get('page', 1);
            $offset = ($page - 1) * $perPage;

            $items = self::orderBy('last_ai_chat_access_at', 'desc')
                ->skip($offset)
                ->take($perPage)
                ->get();
        ...
        /**
         * Cache for hubungan data to prevent N+1 queries
         */
        protected static $hubunganCache = null;

        /**
         * Get cached hubungan data
         */
        protected static function getHubunganCache()
        {
            if (self::$hubunganCache === null) {
                self::$hubunganCache = \App\Models\Hubungan::pluck('hubungan', 'kode_hubungan')->toArray();
            }
            return self::$hubunganCache;
        }
        ...
        public function getHubunganLabelAttribute()
        {
            if ($this->tipe_pengguna === 'keluarga' && $this->kode_hubungan) {
                $hubunganCache = self::getHubunganCache();
                return $hubunganCache[$this->kode_hubungan] ?? $this->kode_hubungan;
            }
            return '-';
        }
